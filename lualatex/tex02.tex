...
\begin{codebox}{php}{onclick-plugin.php}
<?php
/*
 @wordpress-plugin 
 Plugin Name: Onclick Plugin 
 Description: 'onclick'のテスト。ショートコードは [insert_onclick] です。
 Version: 1.0 
...
\verb!                   plugin_dir_url( __FILE__ ) . 'js/onclick.js',! \\
...
\begin{enumerate}
 ...
 \item plugin\_dir\_url( \_\_FILE\_\_ ) . 'js/onclick.js' \\
       読み込むファイルを指定。\textsf{plugin\_dir\_url()} を使うことで、プラグインのフォルダを指定できる。
       \textsf{\_\_FILE\_\_} 指定により、このPHPファイルのあるフォルダを取得でき、
       それを親ディレクトリとして 'js/onclick.js' を指定できる。
 \item array('jquery') \\
...
\begin{tabular}{|l|} \hline
 ...
 \verb!                  plugin_dir_url( __FILE__ ) . 'css/onclick.css',! \\
...
\begin{enumerate}
 ...
 \item plugin\_dir\_url( \verb!__FILE__! ) . 'css/onclick.css' \\
       onclick.css を プラグインフォルダに続けて読み込ませる。
 \item array() \\
...
?>
 <section>
   ...
   <div id="area">
     <img id="close"
          src="<?php echo plugin_dir_url( __FILE__ ); ?>img/close.gif"
          alt="close">
...
\begin{description}
 ...
 \item[7行目] <?php echo plugin\_dir\_url(\verb! __FILE__ !); ?>img/close.gif  \\
       close.gif を plugin\_dir\_url() を使ってディレクトリ指定している。
 \item[13行目] return \verb!ob_get_clean()! \\
       ここでバッファリングしていた文字列を出力する。
       そのことで、'insert\_onclick' という
...
\begin{codebox}{javascript}{onclick.js}
'use strict';
...
今回は、プラグイン・フォルダに直接コードを書いたが、
このプラグインをプラグインファイルとしてアップロード可能にするには、
圧縮ファイルにするとよい。

この onclick-plugin フォルダを zip形式で圧縮して、たとえば ``onclick-plugin.zip''
として適当な場所に保存しておく。
